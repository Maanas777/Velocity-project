...
function Profile() {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3003/api/users/editProfile/${userId}`, updatedUserData);
              ...
              <div className="card mb-3" style={{ borderRadius: '.5rem' }}>
                <div className="row g-0">
                  <div className="col-md-4 gradient-custom text-center text-white"
                    ...
                    <img src={profile} style={{ width: '9rem', height: '13rem', paddingTop: '20%' }} alt="" />
                    <h5 >{username}</h5>

                    {!isEditing && <button className="btn btn-primary" onClick={handleEditClick}>Edit Profile</button>}
                  </div>
